class XIIIMenuLivePlayerMenuSelectedGamerTag extends XIIILiveWindow;

var localized string TitleText;

var XIIIGUIButton Buttons[6];
var localized string ButtonNames[6];
var localized string strTitle, strVoiceOn, strVoiceMuted, strAcceptFriendRequestQuestion, strDeclineFriendRequestQuestion, strBlockFriendRequestQuestion, strKickPlayerQuestion;
...
*/
  if (xboxlive.IsHost() && xboxlive.IsPlayerNameInGame(activeFriendName))
    Buttons[4].bVisible=true;
  else
    Buttons[4].bVisible=false;

  if (xboxlive.IsFriend(activeFriendName))
    Buttons[5].bVisible=false;
  else
    Buttons[5].bVisible=true;

  if (activeFriend.voiceStatus == eVoiceStatus.VOICESTATUS_voicemuted)
    Buttons[3].Caption = strVoiceMuted;//"Voice is muted";
  else
    Buttons[3].Caption = strVoiceOn;//Voice is on;
}
...
function ReturnMsgBox(byte bButton)
{
  switch (bButton)
  {
    case QBTN_Ok:
      ...
      if (popupStatus == 1)
      {
        xboxlive.AcceptFriendRequest(activeFriendName);
        myRoot.CloseMenu(true);
      }
      else if (popupStatus == 2)
      {
        xboxlive.DeclineFriendRequest(activeFriendName);
        myRoot.CloseMenu(true);
      }
      else if (popupStatus == 3)
      {
        xboxlive.BlockFriendRequest(activeFriendName);
        myRoot.CloseMenu(true);
      }
      else if (popupStatus == 3)
      {
...
function bool InternalOnClick(GUIComponent Sender)
{
    ...
    if (Sender == Controls[0])
    {
      ...
      msgbox.SetupQuestion(strAcceptFriendRequestQuestion, QBTN_Ok | QBTN_Cancel, QBTN_Cancel, activeFriendName);
...
//      myRoot.CloseMenu(true);
    ...
    else if (Sender == Controls[1])
    {
      ...
      Controller.OpenMenu("XIDInterf.XIIILiveMsgBox",false);
      msgbox = XIIILiveMsgBox(myRoot.ActivePage);
      msgbox.SetupQuestion(strDeclineFriendRequestQuestion, QBTN_Ok | QBTN_Cancel, QBTN_Cancel, activeFriendName);
      msgbox.OnButtonClick=ReturnMsgBox;
      msgbox.InitBox(160*fRatioX, 130*fRatioY*fScaleTo, 16, 16, 320*fRatioX, 230*fRatioY*fScaleTo);
...
//      myRoot.CloseMenu(true);
    ...
    {
      // No more requests from him
      Controller.OpenMenu("XIDInterf.XIIILiveMsgBox",false);
      msgbox = XIIILiveMsgBox(myRoot.ActivePage);
      msgbox.SetupQuestion(strBlockFriendRequestQuestion, QBTN_Ok | QBTN_Cancel, QBTN_Cancel, activeFriendName);
      msgbox.OnButtonClick=ReturnMsgBox;
      msgbox.InitBox(160*fRatioX, 130*fRatioY*fScaleTo, 16, 16, 320*fRatioX, 230*fRatioY*fScaleTo);
