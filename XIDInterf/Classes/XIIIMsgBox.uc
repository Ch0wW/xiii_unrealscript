...
class XIIIMsgBox extends XIIIWindow;
...
function PaintGfxBackground(Canvas C, float X, float Y)
{
  ...
  if (ShowWorking)
  {
    ...
    DrawStretchedTexture(C, WinLeft+MarginWidth/2+(WinWidth-tWorking[index].USize)/2, WinTop+MarginHeight/2+30, tWorking[index].USize, tWorking[index].VSize, tWorking[index]);
  }
...
function AdjustPosition(Canvas C)
{
	local int i;
	local float left, btnw, BetweenBtnW;

	// adjust message box position
	WinLeft = (C.ClipX - WinWidth)/2;
	WinTop = (C.ClipY - WinHeight)/2;

	// adjust title text position
	Controls[0].WinLeft = WinLeft;
	Controls[0].WinTop = WinTop + MarginHeight;

	// adjust main text position
	Controls[1].WinLeft = WinLeft + MarginWidth;
	Controls[1].WinTop = WinTop + WinHeight/2 - lMessage.WinHeight/2 + lMessage.Style.BorderOffsets[1] + lMessage.Style.BorderOffsets[1] + 10;

	// adjust buttons position
    btnw = 88;      // required width to fit "Abbrechen"
    BetweenBtnW = (WinWidth - Buttons.Length * btnw) / (Buttons.Length+1);
    left = WinLeft + BetweenBtnW;
	for (i=0;i<Buttons.Length;i++)
	{
		Buttons[i].WinLeft = left;
		Buttons[i].WinTop = WinTop + WinHeight - Buttons[i].ActualHeight() - 2*MarginHeight;
        left += (btnw + BetweenBtnW);
	}
}
...
{
    local int i;
    local float left, BetweenBtnW, btnw;

    btnw = 88;      // required width to fit "Abbrechen"
    BetweenBtnW = (WinWidth - Buttons.Length * btnw) / (Buttons.Length+1);
    left = WinLeft + BetweenBtnW;
    log("WinWidth="$WinWidth$", btnw="$btnw$", BetweenBtnW="$BetweenBtnW);
