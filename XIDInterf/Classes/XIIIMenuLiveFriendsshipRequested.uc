class XIIIMenuLiveFriendsshipRequested extends XIIILiveWindow;

var localized string TitleText;

var XIIIGUIButton Buttons[3];
var localized string ButtonNames[3];
var localized string strFriendshipRequested, strAcceptFriendInviteQuestion, strDeclineFriendInviteQuestion, strBlockFriendQuestion;
...
function ShowWindow()
{
  ...
  TitleText = activeFriendName $ strFriendshipRequested;
}
...
function bool InternalOnClick(GUIComponent Sender)
{
    ...
    if (Sender == Controls[0])
    {
      ...
      msgbox.SetupQuestion(strAcceptFriendInviteQuestion, QBTN_Ok | QBTN_Cancel, QBTN_Cancel, activeFriendName);
...
//      myRoot.CloseMenu(true);
    ...
    else if (Sender == Controls[1])
    {
      Controller.OpenMenu("XIDInterf.XIIILiveMsgBox",false);
      msgbox = XIIILiveMsgBox(myRoot.ActivePage);
      msgbox.SetupQuestion(strDeclineFriendInviteQuestion, QBTN_Ok | QBTN_Cancel, QBTN_Cancel, activeFriendName);
      msgbox.OnButtonClick=ReturnMsgBox;
      msgbox.InitBox(160*fRatioX, 130*fRatioY*fScaleTo, 16, 16, 320*fRatioX, 230*fRatioY*fScaleTo);
...
//      myRoot.CloseMenu(true);
    ...
    else if (Sender == Controls[2])
    {
      Controller.OpenMenu("XIDInterf.XIIILiveMsgBox",false);
      msgbox = XIIILiveMsgBox(myRoot.ActivePage);
      msgbox.SetupQuestion(strBlockFriendQuestion, QBTN_Ok | QBTN_Cancel, QBTN_Cancel, activeFriendName);
      msgbox.OnButtonClick=ReturnMsgBox;
      msgbox.InitBox(160*fRatioX, 130*fRatioY*fScaleTo, 16, 16, 320*fRatioX, 230*fRatioY*fScaleTo);
